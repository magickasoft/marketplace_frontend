...
import StoriesReview from 'components/Stories/Review';
import {
    showDatetimePicker,
    hideDatetimePicker,
    changePublishDate,
...
class StoriesReviewContainer extends Component {
    changeQuery(mPage, mSort, mOrder) {
        const { router, location } = this.props;

        router.push({
            pathname: '/content/stories/review',
            query: {
...
StoriesReviewContainer.propTypes = {
...
function select(state) {
    ...
    return {
        storiesList: {
            ...ui.storiesList.review,
            results: ui.storiesList.review.results.map(
                selectResults(state)
            ),
            currentPage: ui.storiesList.review.query.map(property('page')),
            pageCount: ui.storiesList.review.total
                .map(calcPageCount)
